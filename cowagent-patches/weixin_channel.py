...
class WeixinChannel(ChatChannel):
    ...
    def _send_text(self, text: str, receiver: str, context_token: str):
        # Split by ||| for step-by-step messaging (natural multi-message flow)
        if "|||" in text:
            parts = [p.strip() for p in text.split("|||") if p.strip()]
            for i, part in enumerate(parts):
                self._send_single_text(part, receiver, context_token, f"part {i+1}/{len(parts)}")
                if i < len(parts) - 1:
                    time.sleep(1.0)  # natural pause between messages
            return
        self._send_single_text(text, receiver, context_token)

    def _send_single_text(self, text: str, receiver: str, context_token: str, label: str = ""):
        if len(text) <= TEXT_CHUNK_LIMIT:
            try:
                resp = self.api.send_text(receiver, text, context_token)
                self._check_send_response(resp, receiver)
                logger.debug(f"[Weixin] Text sent to {receiver}, len={len(text)}{' (' + label + ')' if label else ''}")
            except Exception as e:
