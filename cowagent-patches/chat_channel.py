...
class ChatChannel(Channel):
    ...
    def _extract_and_send_images(self, reply: Reply, context: Context):
        ...
        if media_items:
            ...
            # Strip markers from text so user doesn't see raw [图片: ...]
            for pattern, _ in patterns:
                reply.content = re.sub(pattern, '', reply.content)
            reply.content = reply.content.strip()

            # Send remaining text (if any) before media
            if reply.content:
                ...
                try:
                    ...
                    if url.startswith(('http://', 'https://')):
                        ...
                        if os.path.isabs(url):
                            ...
                            resolved = os.path.join(workspace, url)
                        if os.path.exists(resolved):
                            if media_type == 'video':
                                media_reply = Reply(ReplyType.FILE, f"file://{resolved}")
                                media_reply.file_name = os.path.basename(resolved)
                            else:
                                media_reply = Reply(ReplyType.IMAGE_URL, f"file://{resolved}")
                        else:
                            logger.warning(f"[chat_channel] Media file not found: {url} (resolved: {resolved})")
                            continue
